...
describe('Gilded Rose system for standard products', () => {
  ...
  it('should lower the SellIn & Quality values by 1 each _day', () => {
  ...
  it('should lower the SellIn & Quality values linearly after multiple _days', () => {
    range(4).forEach(_day => gildedRose.updateQuality());
    const updatedStandardItem = gildedRose.items[0];
  ...
  it('should lower the Quality value twice as fast when SellIn value is negative', () => {
    range(6).forEach(_day => gildedRose.updateQuality());
  ...
  it('should never lower the Quality value under 0, no matter how many _days have passed after the SellIn _day', () => {
    range(10).forEach(_day => gildedRose.updateQuality());
    const updatedStandardItem = gildedRose.items[0];
  ...
  it('should not increase a Quality value above 50 for products whose quality increase with time', () => {
    ...
    range(45).forEach(_day => gildedRose.updateQuality());
...
describe('Gilded Rose system for non-standard products', () => {
  it('should increase the Quality value of "Aged Brie" the older it gets, by 2 when over the SellIn date', () => {
    ...
    range(3).forEach(_day => gildedRose.updateQuality());
  ...
  it('should increase the Quality value of "Backstage passes" the older it gets, by 2 when 10 _days or less left, by 3 when 5 _days or less left, and drops it at 0 after concert', () => {
    ...
    range(2).forEach(_day => gildedRose.updateQuality());

    expect(gildedRose.items[0].sellIn).toBe(10);
    expect(gildedRose.items[0].quality).toBe(25);

    range(5).forEach(_day => gildedRose.updateQuality());
